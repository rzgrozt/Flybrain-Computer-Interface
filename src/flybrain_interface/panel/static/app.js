...
function drawRate(){const names=[...new Set(history.flatMap(t=>Object.keys(t.population_rates_hz||{})))];plot('rateChart',names.map(n=>({values:history.map(t=>t.population_rates_hz?.[n]||0)})));$('legend').innerHTML=names.map((n,i)=>`<span><i style="background:${colors[i%colors.length]}"></i>${escapeHtml(n)}</span>`).join('')||'No population samples yet.'}
...
function selectedValues(){const targets=parseIndices($('targets').value,{required:true,label:'Target neurons'});const watch=parseIndices($('watch').value,{label:'Watchlist'});const outputs=selectedPreset?selectedPreset.outputs.flatMap(x=>x.neuron_indices):targets;return {targets,watch,outputs,visualization:[...new Set([...targets,...watch,...outputs])].sort((a,b)=>a-b)}}

$('start').onclick=()=>{let values;try{values=selectedValues();if(values.visualization.length>4096)throw Error('Brain View selection exceeds 4,096 neurons.')}catch(e){$('error').textContent=e.message;return}const duration=Number($('duration').value),stop=Number($('stimStop').value),populations=selectedPreset?selectedPreset.outputs.map(x=>({name:x.label.slice(0,40),neuron_indices:x.neuron_indices})):[{name:'stimulus targets',neuron_indices:values.targets}];history=[];void refreshSelection(values.targets,values.outputs,values.watch);action('/api/experiments',{duration_s:duration,seed:Number($('seed').value),backend:'numba',subnormal_drive_policy:$('policy').value,chunk_duration_s:0.02,telemetry_hz:10,watch_indices:values.watch,visualization_indices:values.visualization,populations,stimulus:{neuron_indices:values.targets,start_s:0,stop_s:stop,interval_ms:Number($('interval').value),amplitude_mv:Number($('amplitude').value)}})};
...
async function loadPresets(){try{const r=await fetch('/api/presets/sensory-descending');if(!r.ok)throw Error('Preset metadata unavailable.');const p=await r.json();p.inputs.forEach((x,i)=>{$('preset').add(new Option(`${x.label} (${x.selected_count}/${x.available_count})`,String(i)))});$('preset').onchange=()=>{if($('preset').value==='manual'){selectedPreset=null;$('presetNote').textContent='Manual engineering stimulation and observation assignments.';return}const input=p.inputs[Number($('preset').value)];selectedPreset={input,outputs:p.outputs};$('targets').value=input.neuron_indices.join(',');$('watch').value=p.outputs.filter(x=>x.key!=='all_descending').flatMap(x=>x.neuron_indices).join(',');$('presetNote').textContent=`${input.label}: ${input.selected_count} of ${input.available_count}, selected by the documented stable-body-ID rule. Output charts use annotated descending groups; no behavioral function is implied.`;void refreshSelection(input.neuron_indices,p.outputs.flatMap(x=>x.neuron_indices),parseIndices($('watch').value,{label:'Watchlist'}))}}catch(e){$('presetNote').textContent=e.message}}
