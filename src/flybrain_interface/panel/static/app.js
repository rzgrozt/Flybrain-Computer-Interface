...
let observedPathway=window.flybrainSelectedPathway||null;
window.addEventListener('flybrain:pathway-selection',event=>{observedPathway=event.detail;});
window.addEventListener('flybrain:pathway-overlay',event=>{brain.setPathway(event.detail||null);});
if(window.flybrainPathwayOverlay)brain.setPathway(window.flybrainPathwayOverlay);

function statusName(s){return s?String(s).replaceAll('_',' '):'idle'}
function update(t){
  const s=t.status||'idle';
  window.dispatchEvent(new CustomEvent('flybrain:simulation-status',{detail:{status:s,experiment_id:t.experiment_id||null}}));
...
function connect(){socket=new WebSocket(`${location.protocol==='https:'?'wss':'ws'}://${location.host}/ws/telemetry`);socket.onopen=()=>{$('connection').textContent='live';$('activityState').classList.remove('stale')};socket.onmessage=e=>update(JSON.parse(e.data));socket.onclose=()=>{$('connection').textContent='reconnecting';$('activityState').textContent='disconnected · activity cleared';$('activityState').classList.add('stale');brain.updateActivity(null);window.dispatchEvent(new Event('flybrain:neural-disconnected'));setTimeout(connect,1000)}}
setInterval(()=>{if(activityIsStale(lastTelemetryAt,performance.now())&&$('connection').textContent==='live'){$('activityState').textContent='stale · latest activity cleared';$('activityState').classList.add('stale');brain.updateActivity(null);window.dispatchEvent(new Event('flybrain:neural-disconnected'))}const p=brain.getPerformance();$('renderPerf').textContent=`WebGL CPU submit: ${p.mean_cpu_submit_ms.toFixed(2)} ms mean, ${p.max_cpu_submit_ms.toFixed(2)} ms max over ${p.render_count.toLocaleString()} draws (GPU completion not measured).`},500);
